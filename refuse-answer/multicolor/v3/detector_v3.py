...
from config_v3 import MAX_CLUSTER_PIXELS, MIN_ROI_PIXELS, SAMPLE_SEED
from purity_rules_v3 import decide_purity
...
def detect_region(image_rgb, mask, region_id, region_name="region"):
    ...
    stats = cluster_lab_pixels(pixels_lab)
    decision = decide_purity(stats)
    return _format_result(region_name, n_pixels, decision, stats, len(sampled_pixels_rgb))
...
def _format_result(region_name, n_pixels, decision, stats, sampled_pixels=0):
    stats = stats or {
        "main_ratio": 0.0,
        "second_ratio": 0.0,
